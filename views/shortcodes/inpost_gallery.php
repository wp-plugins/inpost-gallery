...
<?php if (!empty($slides) AND is_array($slides)): ?>
    <div class="yoxview" id="yoxview_<?php echo $unique_id ?>">
        <?php foreach ($slides as $gallery_item) : ?><a style="<?php echo $styles_a ?>" href="<?php echo $gallery_item['imgurl'] ?>" id="yoxview_img_<?php echo($counter++);  ?>"><img style="<?php echo $styles_img ?>" src="<?php echo PluginusNet_InpostGallery::get_image($gallery_item['imgurl'], $thumb_width . 'x' . $thumb_height) ?>" alt="<?php echo $gallery_item['title'] ?>" title="<?php echo $gallery_item['title'] ?>" /></a><?php endforeach; ?>
    </div>
<?php endif; ?>
