...
<ul class="misc-pub-section" style="border-top: none;">
	<li><input type="checkbox" id="random_shortcode" class="change_inpost_shortcode" value="1" />&nbsp;<?php _e("Random", 'inpost-gallery') ?>
		...
		&nbsp;
		<input type="text" value="" class="bg_hex_color text" id="color_shortcode"><span style="background-color: #fff;" class="bgpicker"></span>&nbsp;<?php _e("Border color", 'inpost-gallery') ?>
	</li>
</ul>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		...
		//***
		jQuery("#color_shortcode").on('keydown', function() {
			update_inpost_shortcode_text();
		});
	});
	...
	function update_inpost_shortcode_text() {
		...
		//*** color
		var border = "";
		if (jQuery("#color_shortcode").val().length > 0) {
			border = 'solid 1px ' + jQuery("#color_shortcode").val();
			shortcode_text = shortcode_text + ' border="' + border + '"';
		}
		//*****
