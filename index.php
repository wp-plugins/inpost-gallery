...
define("INPOST_GALLERY_UPLOADS_DIR", WP_CONTENT_DIR."/uploads/inpost-gallery-uploads/");
define("INPOST_GALLERY_UPLOADS_DIR_LINK", home_url()."/wp-content/uploads/inpost-gallery-uploads/");
...
if (isset($_REQUEST['inpostgallery_admin_ajax_action'])) {
    ...
    if ($wp_user_level == 10) {
        ...
        switch ($_REQUEST['inpostgallery_admin_ajax_action']) {
            ...
            case 'up_image':
                $controller->model->resort_images($_REQUEST['image_id'], $_REQUEST['previous_image_id']);
                break;

            case 'down_image':
                $controller->model->resort_images($_REQUEST['image_id'], $_REQUEST['next_image_id']);
                break;
...
function inpostgallery_setup() {
    global $wpdb;
    if ($wpdb->get_var("SHOW TABLES LIKE '%inpostgallery%'") == 'inpostgallery') {
        $data = $wpdb->get_results("SHOW COLUMNS FROM inpostgallery");
        $sort_added = false;
        ...
        if (!$sort_added) {
            $wpdb->query("ALTER TABLE inpostgallery ADD sort INT(11) NOT NULL");
            $wpdb->query("UPDATE `inpostgallery` SET `sort` = `id`");
        }
    } else {
...
function inpostgallery_admin_head() {
    ...
    wp_enqueue_script('inpostgallery_admin', plugins_url('js/admin.js', __FILE__), array('jquery'));
}
...
function inpostgallery_draw_post_panel() {
    ...
    if ($post_id > 0) {
        ...
        echo inpostgallery_helper_localize('save_before');
    }
}
