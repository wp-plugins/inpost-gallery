...
<?php if (!empty($gallery_data)): ?>
    <div class="yoxview">
        <?php foreach ($gallery_data as $image) : ?>
           <a href="<?= INPOST_GALLERY_PLUGIN_LINK . "uploads/" . $image['post_id'] . "/" . $image['image_file_name'] ?>"><img class="inpostgallery_image_<?= $image['id'] ?>" src="<?= INPOST_GALLERY_PLUGIN_LINK . "uploads/" . $image['post_id'] . "/thumbs/" . $image['image_file_name'] ?>" alt="<?= $image['image_title'] ?>" title="<?= $image['image_title'] ?>" /></a>
        <?php endforeach; ?>
    </div>
<?php endif; ?>
